...
| Read replica | Explicit, not automatic: `Account.reading` for reports and data exports only; tills, the live dashboard and everything that writes stay on the primary. Rails' automatic role switching is off because the row-level security settings are per connection. Development and test have a replica pointing at the same database so the code path always runs. |
| Scaling out | A Kamal destination (`deploy.scaled.yml`) rather than a second deploy file: two web servers, a jobs server, a primary and a streaming replica (pg_basebackup through a slot, capped by `max_slot_wal_keep_size`), all on the private network. Uploads move to S3-compatible object storage. PgBouncer waits until four web servers and needs `SET LOCAL` first. |
...
| Loyalty points | One scheme per shop: points per 100 spent (rounded down), a value per point, a minimum to spend. A ledger of entries (earned, redeemed, reversed, adjusted) per customer, never edited. Earned when a named customer's sale completes, including offline sales when they sync, on the total less points paid. Spent as a tender, taken off when the payment is made and given back if it's removed. Voids reverse the sale's net points; returns take back their share of the earned points. Points don't expire; eTIMS reports points as "other". |
| Offline parity | The till's snapshot carries customers (name, phone, price list), price-list prices and promotions running or starting within 14 days; the client repeats the server's price-list and promotion rules (shared test cases in Ruby and JS) and checks promotion dates in the shop's time zone. The server records the client's prices, as before, and flags a promotion that wasn't running that day. Spending points, accounts and hand discounts still need the till online. Customer names and phones on till devices are covered by the devices being the shop's own (docs/SECURITY.md). |
...
| 21 — Offline till parity | **Done:** customers and their price lists on the offline till; promotions applied offline with dates checked locally; points earned on sync; receipts with customer and savings; promotion-date warnings; snapshot versioning to the microsecond; checked in a browser with the server stopped. |
| 20 — Loyalty points | **Done:** points scheme in Settings with liability and top customers; earning on completed named sales (offline ones too); points tender at the till with balance shown; void and return reversals; receipts (and ESC/POS) with points earned and balance; customer points history with adjustments by managers. |
...
**Goal:** move from one VPS to several without a rewrite, when the load asks for it.
- **Read replica** for reports and exports, with row-level security on its connection.
- **Kamal destination** for separate database, replica, web and job servers; replica bootstrap.
- **Object storage** for uploads, with a task to move existing files.
- **Runbook** for when and how to grow, the connection budget, and going back.

### Phase 18 — Foreign currencies
**Goal:** border-town shops take dollars and neighbouring shillings, and importers buy in dollars, without a second
set of books.
- **Currencies and rates** in Settings, kept by owners and managers.
...
**Goal:** give regulars a reason to come back, without a card system.
- **Points scheme** in Settings; earning on sales to named customers.
- **Spending at the till** as a tender; reversals on voids and returns.
- **Balances** on the till, receipts and the customer's page, with adjustments.

### Phase 21 — The offline till catches up
**Goal:** a sale shouldn't cost a contractor more, or earn them less, because the internet was down.
- **Customers and price lists** in the till's snapshot, chosen offline.
- **Promotions** applied offline with the same rules, dates checked on the day.
- **Points** earned when the sale syncs; warnings for anything worth a second look.
...
- FIFO costing; points expiry and points by text message; spending points offline; mix-and-match bundles. (~~Multi-currency~~ done in Phase 18; a daily rate feed and exchange gain/loss are possible extras.) (~~Batch/lot tracking~~ done in Phase 16; counting by batch is a possible extra.)
